...
namespace mf
{
	...
	template <typename DataType>
	void LinkedList<DataType>::insert(int index, const DataType& data)
	{
		if (size == 0)
		{
			PushFront(data);
		...
		ListNode<DataType>* newListNode = new ListNode<DataType>();
		...
		// Push in middle
		ListNode<DataType>& temp = (*this)[index - 1];
	...
	void LinkedList<DataType>::remove(const DataType& data)
	{
		...
		while (true)
		{
			if (cur->next == nullptr)
			{
				throw "There is no such data in list";
			}
